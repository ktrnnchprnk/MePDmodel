...
% plot3(Y1(locs1(1):locs1(end),1),Y1(locs1(1):locs1(end),3), Y1(locs1(1):locs1(end),6)/60, 'LineWidth', 1, 'Color','k')
scatter3(ColoredOut1(:,5), ColoredOut1(:,6),log(ColoredOut1(:,2)/60),8, color_vector(:, :),'o', 'filled');
% fill3(ColoredOut1(:,5), ColoredOut1(:,6),ColoredOut1(:,2)/60,'b', 'FaceAlpha', 0.1);

% title('Control')
% subtitle(PeriodMain(1))

% xlabel('$G_l$', 'Interpreter','latex')
% ylabel('$G_e$', 'Interpreter','latex')
% zlabel('KNDy activity (Hz)')
% xlim([0 0.2])
% zlim([-2 2])
% zlim([0 max(Y(:,6)/60)+5])
view(220, 20)
% plot2svg('sphere.svg',11);
hold off
% fig2svg('sphere.svg');
